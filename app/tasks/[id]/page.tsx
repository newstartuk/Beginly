    ...
    <PlatformShell title={task.title} eyebrow={`Task Library · ${task.stage} · ${task.category}`}>
      <div className="space-y-5 animate-fade-in max-w-3xl">
        ...
        <Link href="/checklist" className="inline-flex items-center gap-1 text-sm text-muted hover:text-primary transition-colors">
          <ChevronLeft className="w-4 h-4" /> Back to task library
        </Link>
        ...
        {officialLinks.length > 0 && (
          <div className="card">
            <h2 className="section-title">
              <ExternalLink className="w-4 h-4" style={{ color: "var(--color-teal)" }} />
              Useful links
            </h2>
            <div className="flex flex-wrap gap-2">
              {officialLinks.map((resource) => (
                resource.url.startsWith("/") ? (
                  <Link
                    key={`${task.taskId}-${resource.url}`}
                    href={resource.url}
                    className="inline-flex items-center gap-1.5 rounded-full border border-border px-3 py-2 text-sm font-medium text-navy transition-colors hover:border-primary hover:text-primary"
                  >
